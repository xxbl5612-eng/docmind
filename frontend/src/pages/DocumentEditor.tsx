...
export default function DocumentEditor() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        const { data } = await aiApi.taskStatus(id, taskId);
  ...
  const runAiTool = async () => {
    ...
    try {
      ...
      if (doc && doc.char_count && doc.char_count > 20000) {
        let res;
        switch (activeTool) {
          case 'proofread': res = await aiApi.asyncProofread(id, payload); break;
          case 'rewrite': res = await aiApi.asyncRewrite(id, payload); break;
          case 'summarize': res = await aiApi.asyncSummarize(id, payload); break;
          case 'extract': res = await aiApi.asyncExtract(id, payload); break;
          case 'convert': res = await aiApi.asyncConvert(id, payload); break;
          default: res = await aiApi.asyncSummarize(id, payload);
        }
        ...
        let res;
        switch (activeTool) {
          case 'proofread': res = await aiApi.proofread(id, payload); break;
          case 'rewrite': res = await aiApi.rewrite(id, payload); break;
          case 'summarize': res = await aiApi.summarize(id, payload); break;
          case 'extract': res = await aiApi.extract(id, payload); break;
          case 'convert': res = await aiApi.convert(id, payload); break;
          case 'qa': res = await aiApi.qa(id, payload); break;
    ...
    <div className="flex h-[calc(100vh-4rem)]">
      <div className="w-64 border-r border-surface-200 bg-white flex flex-col flex-shrink-0 overflow-y-auto">
        ...
        <div className="p-4 border-t border-surface-200 space-y-2">
          <Link to={`/documents/${id}/versions`} className="block text-sm text-surface-600 hover:text-primary-600">{t('editor.version_history')}</Link>
          <Link to={`/documents/${id}/collaboration`} className="block text-sm text-surface-600 hover:text-primary-600">{t('editor.collaboration')}</Link>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          {activeTool && (
            <div className="w-96 border-l border-surface-200 bg-white flex flex-col overflow-y-auto flex-shrink-0">
              <div className="p-4 border-b border-surface-200">
                <h3 className="font-semibold text-surface-900 capitalize">{t(`editor.${activeTool}`)}</h3>
              </div>
                  ...
                  <div className="bg-white rounded-lg border border-surface-200">
                    <div className="flex border-b border-surface-200">
                      <button
                        ...
                        className={`flex-1 px-3 py-2 text-xs font-medium transition-colors cursor-pointer ${resultTab === 'result' ? 'text-primary-700 border-b-2 border-primary-500' : 'text-surface-500 hover:text-surface-700'}`}
                      >
                        {t('editor.result')}
                      </button>
                      {resultStats && Object.keys(resultStats).length > 0 && (
                        <button
                          ...
                          className={`flex-1 px-3 py-2 text-xs font-medium transition-colors cursor-pointer ${resultTab === 'stats' ? 'text-primary-700 border-b-2 border-primary-500' : 'text-surface-500 hover:text-surface-700'}`}
                        >
                          {t('editor.stats')}
                        </button>
                      )}
