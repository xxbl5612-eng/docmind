...
export default function Register() {
  ...
  const { register, loginWithGitHub, isLoadingGitHub } = useAuth();
  ...
  const handleGitHubSignUp = async () => {
    try {
      ...
      toast(t('auth.github_login_failed'), 'error');
    }
  };
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-white rounded-xl border border-surface-200 p-6 space-y-4 shadow-sm">
          ...
          <div className="relative">
            ...
            <div className="relative flex justify-center text-xs"><span className="bg-white px-2 text-surface-400">{t('auth.or_continue_with')}</span></div>
          </div>
          <Button variant="outline" onClick={handleGitHubSignUp} loading={isLoadingGitHub} className="w-full">
